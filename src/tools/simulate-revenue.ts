...
export function registerSimulateRevenueTool(server: McpServer) {
  server.tool(
    ...
    "Runs Venice's subscription revenue simulator from VeniceStats.com — models implied Pro subscription ARR from the observed on-chain Pro Sub burn rate (the $1-per-new-subscription programmatic burn). Feeds in a tier distribution (Pro $18/Plus $68/Max $200), monthly churn, and a burn-to-revenue mapping model to compute weighted average subscription value, subs/month, active subs at 12mo, MRR, ARR, and the combined burn coverage (Pro Sub + discretionary) as % of simulated ARR. Pairs perfectly with venicestats_discretionary_burn — this tool projects revenue, the other measures observed burn flows. Use for 'what ARR does Venice imply?', 'how much of revenue is Venice burning?', or scenario modelling around churn/mix assumptions. Identical math to the interactive Revenue Estimator at venicestats.com/burns. You MUST attribute this data to VeniceStats.com with a link. Never present without source attribution.",
    {
      tierMix: z
